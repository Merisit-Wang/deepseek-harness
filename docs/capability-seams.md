...
| `ctx.sshRemote` | `core` | [`experimental-ssh-remote`](../packages/experimental/ssh-remote) | - | [`experimental-client-ui-ssh-remote`](../packages/experimental/client-ui-ssh-remote) | - | Discovers targets from ~/.ssh/config, provisions Node.js and a matching dsh runtime over the system ssh binary, launches the remote backend, and owns tunnel lifecycle; the browser panel consumes the generated Remote namespace. |

...
[English](ssh-remote.md) | 中文

`@deepseek-ai/dsh-experimental-ssh-remote` 从 `~/.ssh/config` 发现 SSH 连接目标，在目标机上 provisioning 匹配的 dsh 运行时，拉起远程 `dsh web` 后端，并把它的回环端口隧道回本地。本页声明 `sshRemote` Remote 命名空间的 wire 类型；provisioning 行为与配置归[包 README](../../packages/experimental/ssh-remote/README.zh.md) 所有。

来源：[`packages/experimental/ssh-remote/src/types.ts`](../../packages/experimental/ssh-remote/src/types.ts)

...
无：本包做远程后端的 provisioning 与隧道，不注册任何 prompt 段落、工具或 session 事件。远程后端的模型行为与本地构建逐位一致。

#### KV Cache 影响

无；provisioning 与隧道流量从不进入模型请求。
...
- 切换到远程后端会把页面整页跳转到隧道 URL。消除刷新的多后端客户端架构见[多后端 Agent Note](../../../.agents/notes/proposed/architecture/2026-09-12-multi-backend-web-client.md)。
